...
.then((value) => {
    ...
    let write = '';
    for (key in value2) {
        console.log(value2[key].bpi)

        write += `
        <div class="col-md-6 mb-md-0 p-md-4">
            <img src="image/pexels-photo-3531895.jpeg" class="w-100" alt="This is image">
        </div>
        ...
        <div class="col-md-6 p-4 ps-md-0">
            ...
            <p class="para_tag">The USD is the currency abbreviation for the U.S. dollar ($), the official currency of the United States of America and the world's primary reserve currency over the past several decades. It is managed by the Federal Reserve, America's central bank. In foreign exchange (forex) markets, the USD is the most common pairing in exchange with other currencies; for instance, EUR/USD, USD/JPY, and GBP/USD. The U.S. dollar is also the official currency for a small number of other nations such as The Marshall Islands, Panama, and Ecuador, and is unofficially accepted in local exchange in several other countries around the world.</p>
            <div class="info-box">
                <h3 class="all_info">More Info :</h3>
    ...
    document.getElementById('div').innerHTML = write;

    // Remove some unwanted elements
    const div = document.getElementById('div');
    const elementsToRemove = Array.from(div.children).slice(5);
    elementsToRemove.forEach(element => element.classList.add('remove'));
})
.catch((error) => {
